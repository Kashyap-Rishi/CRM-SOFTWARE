...
async function processDeliveryReceiptMessage(data) {
  ...
  try {
    ...
    const customer = communicationLog.customers.find(
      (c) => c.customer.toString() === customerId
    );
    if (!customer) {
      console.error(
        `Customer with ID ${customerId} not found in communication log ${logId}`
      );
      return;
    }
    ...
    console.log(
      `Updated status for customer ${customerId} to ${deliveryStatus}`
    );
  } catch (error) {
