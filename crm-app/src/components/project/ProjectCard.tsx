...
import { Card, CardContent, Typography, Box, Chip, Button, useMediaQuery, Theme, LinearProgress } from '@mui/material';
...
const ProjectCard: React.FC<ProjectCardProps> = ({ projectName, deadline, users, completed, projectId, username, task }) => {
  ...
  const handleMarkAsCompleted = async () => {
    try {
      const response = await axios.put(`http://localhost:8000/api/project/mark-completed/${projectId}`);
      ...
      <CardContent>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1 }}>
            ...
            {overdueTimer && (
              <Typography component="span" sx={{ backgroundColor: '#FB726A', color: 'white', padding: '2px 4px', borderRadius: 4 }}>
                {`${overdueTimer}`}
              </Typography>
            )}
          </Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {users.map(user => (
                <Chip key={user.employeeId} label={user.name} sx={{ margin: 0.5 }} />
              ))}
            </Box>
          </Box>
          ...
          <LinearProgress variant="determinate" value={progressPercentage} sx={{ marginBottom: 2 }} />
          ...
          {!completed && !overdueTimer && (
            <Button
              ...
              sx={{ marginTop: 2,backgroundColor: '#A7DCA5',opacity:0.8, '&:hover': {
                '&:hover': {
