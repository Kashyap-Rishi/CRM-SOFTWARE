...
const validateCustomerData = (data) => {
  ...
  if (
    ...
    (typeof totalSpends !== "number" || totalSpends < 0)
  ) {
    errors.push("'totalSpends' must be a non-negative number");
  }
  if (
    maxVisits !== undefined &&
    (typeof maxVisits !== "number" || maxVisits < 0)
  ) {
    errors.push("'maxVisits' must be a non-negative number");
  }
  if (
    lastVisit !== undefined &&
    (typeof lastVisit !== "number" || lastVisit < 0)
  ) {
...
const validateCommunicationLogData = (data) => {
  ...
  if (customers && Array.isArray(customers)) {
    customers.forEach((customer, index) => {
      if (
        !customer.customer ||
        !mongoose.Types.ObjectId.isValid(customer.customer)
      ) {
        errors.push(`Invalid or missing 'customer' at index ${index}`);
      }
      if (
        !customer.status ||
        !["PENDING", "SENT", "FAILED"].includes(customer.status)
      ) {
        errors.push(
          `Invalid or missing 'status' for customer at index ${index}`
        );
      }
    });
  } else {
