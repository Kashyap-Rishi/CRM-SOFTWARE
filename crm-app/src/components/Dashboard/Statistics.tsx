...
const Statistics: React.FC = () => {
  ...
  if (!contextLogValue || !contextLogValue.data || !contextCustomerValue || !contextCustomerValue.data) {
  ...
  const totalCampaignsSent = campaigns.filter(campaign => campaign.campaignStatus === 'SENT').length;

  const totalSuccessfullyDelivered = campaigns.reduce((acc, campaign) => {
    return acc + campaign.customers.filter((customer:any) => customer.status === 'SUCCESS').length;
  }, 0);

  const uniqueRegisteredUsers = customers.length;

  const totalPendingCampaigns = campaigns.filter(campaign => campaign.campaignStatus === 'PENDING').length;
  ...
  const dailyVisits = customers.reduce((acc, customer) => acc + customer.maxVisits, 0);
  ...
  {[
    ...
    { title: 'Successfully Delivered', value: totalSuccessfullyDelivered },
    ...
    <Grid item xs={12} sm={6} md={2.4} key={index}>
      <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: 2 }}>
        <CardContent>
          <Typography variant="h6" align= 'center' style={{ fontSize: isMobile ? '1.5rem' : '1.2rem', marginBottom: '8px' }}>
            {card.title}
          </Typography>
          <Typography variant="h4" align="center" style={{ fontSize: isMobile ? '2rem' : '2.5rem' }}>
            {card.value}
          </Typography>
        </CardContent>
      </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={totalSpendsData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="totalSpends" stroke="#7AB2B2" />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
